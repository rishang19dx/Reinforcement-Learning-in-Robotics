...
def run_monte_carlo(env, num_episodes=NUM_EPISODES, epsilon=EPSILON, gamma=GAMMA, alpha=ALPHA):
    ...
    For each episode:
      1. Generate a complete trajectory using epsilon-greedy policy.
      2. Compute discounted returns G backward from the terminal step.
      3. For each first-visit (state, action), update Q incrementally:
         Q(s,a) += alpha * (G - Q(s,a))
      4. Record the total undiscounted episode reward.
    ...
    for episode in range(num_episodes):
        ...
        # ----- Compute returns & first-visit update -----
        G = 0.0
        visited = set()
        ...
        for state_t, action_t, reward_t in reversed(trajectory):
            ...
            if sa not in visited:
                visited.add(sa)
                # Incremental update (constant-alpha first-visit MC)
                q_table[state_t][action_t] += alpha * (G - q_table[state_t][action_t])

        if (episode + 1) % 50 == 0:
            avg = np.mean(episode_rewards[-50:])
            print(f"MC Episode {episode + 1}/{num_episodes}, Avg Reward: {avg:.2f}")
...
def run_q_learning(env, num_episodes=NUM_EPISODES, epsilon=EPSILON, gamma=GAMMA, alpha=ALPHA):
    ...
    At each step:
      ...
      3. Update: Q(s,a) += alpha * [r + gamma * max_a' Q(s',a') - Q(s,a)]
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 50 == 0:
            avg = np.mean(episode_rewards[-50:])
            print(f"Q-Learning Episode {episode + 1}/{num_episodes}, Avg Reward: {avg:.2f}")
...
def main():
    ...
    env = HoverAviary(obs=ObservationType.KIN, act=ActionType.ONE_D_RPM, gui=False, record=True)
